...
export function getSmartphoneOptimizedVisualSettings(settings) {
  ...
  return {
    ...normalized,
    panelPaddingX: Math.min(normalized.panelPaddingX, 2),
    panelPaddingTop: Math.min(normalized.panelPaddingTop, 2),
    panelPaddingBottom: Math.min(normalized.panelPaddingBottom, 2),
    leftPad: Math.min(normalized.leftPad, 32),
    topPad: Math.min(normalized.topPad, 18),
    rightPad: Math.min(normalized.rightPad, 6),
    bottomPad: Math.min(normalized.bottomPad, 8),
    compactStringGap: Math.min(normalized.compactStringGap, 31),
    standardStringGap: Math.min(normalized.standardStringGap, 35),
    openLaneWidth: Math.min(normalized.openLaneWidth, 24),
    preferredFretWidth: Math.min(normalized.preferredFretWidth, 50),
    minFretWidth: Math.min(normalized.minFretWidth, 30),
